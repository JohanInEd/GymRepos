  ...
  return (
    <div className="rounded-lg border border-gray-200 bg-white p-4">
      <div className="flex flex-col gap-3 border-b border-gray-200 pb-3 sm:flex-row sm:items-start sm:justify-between">
        <div>
          <p className="text-sm font-medium text-gray-500">Mensualidad</p>
          <h3 className="text-lg font-semibold text-gray-950">{member.planName}</h3>
          <p className="text-sm text-gray-500">
            {formatDate(member.startDate)} - {formatDate(member.endDate)}
          </p>
        </div>
        <div className="rounded-lg bg-gray-950 px-4 py-3 text-center text-white">
          <p className="text-2xl font-semibold">{remainingDays}</p>
          <p className="text-xs font-medium uppercase tracking-wide text-gray-300">dias restantes</p>
        </div>
      </div>

      <div className="mt-4">
        <div className="mb-2 flex items-center justify-between text-sm">
          <span className="font-medium capitalize text-gray-700">{monthLabel}</span>
          <span className="text-gray-500">{Math.round(progress)}% usado</span>
        </div>
      ...
      <div className="mt-4 flex flex-wrap gap-3 text-xs text-gray-600">
        ...
        <span className="inline-flex items-center gap-2">
          <span className="h-3 w-3 rounded bg-white ring-2 ring-gray-950" />
          Hoy
        </span>
      </div>
    </div>
